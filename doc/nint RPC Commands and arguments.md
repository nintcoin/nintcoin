# nint RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, nint-qt (or nintd) will look for a file named nint.conf in the nint data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.nint/nint.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/nint.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\nint\nint.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\nint\nint.conf |

Note: if running nint in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of nint Core version 2.0.0:
...
Start nint Core Daemon with option:
<table>
<tr> <td>nintd</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: nint.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: nintd.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the nint and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in nint/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in nint/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all nint specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for nint inputs (0-1, default: 1)
...
This documentation lists all available RPC commands as of nint version 2.0.0, and limited documentation on what each command does. For full documentation of arguments, results and examples, type help ( “command” ) to view full details at the console. You can enter commands either from Tools > Debug console in the QT wallet, or using nint-cli
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | nint )
...
#### preparebudget "proposal-name" "url" payment-count block-start "nint-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "nint-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or nint.conf setting gen) It can also be set with the setgenerate call.
...
#### validateaddress “nintaddress”

Return information about the given nint address.

#### verifymessage “nintaddress” “signature” “message”
...
#### bip38decrypt "nintaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "nintaddress" "passphrase"

Encrypts a private key corresponding to 'nintaddress'.

#### dumpprivkey “nintaddress”

Reveals the private key corresponding to ‘nintaddress’. Then the importprivkey can be used with this output
...
DEPRECATED. Returns the current nint address for receiving payments to this account.
...
Returns a new nint address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new nint address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “nintaddress” ( minconf addlockconf )

Returns the total amount received by the given nintaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “nintprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “tonintaddress” amount ( minconf addlockconf “comment” “comment-to” )

DEPRECATED (use sendtoaddress). Sent an amount from an account to a nint address.
...
#### sendtoaddress “nintaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending nints
...
Usage:
  nint-tx [options] <hex-tx> [commands]  Update hex-encoded nint transaction
  nint-tx [options] -create [commands]   Create hex-encoded nint transaction
