...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::nint);
    ...
    if (!settings.contains("nAnonymizenintAmount"))
        settings.setValue("nAnonymizenintAmount", 1000);

    nAnonymizenintAmount = settings.value("nAnonymizenintAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizenintAmount"))
        SoftSetArg("-anonymizenintamount", settings.value("nAnonymizenintAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nint.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizenintAmount:
            return QVariant(nAnonymizenintAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizenintAmount:
            nAnonymizenintAmount = value.toInt();
            settings.setValue("nAnonymizenintAmount", nAnonymizenintAmount);
            emit anonymizenintAmountChanged(nAnonymizenintAmount);
            break;
        case CoinControlFeatures:
